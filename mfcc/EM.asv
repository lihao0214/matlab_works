...
while 1
    ...
    for i=1:nbStates
        %Compute probability p(x|i)
        Pxi(:,i) = gaussPDF(Data, Mu(:,i), Sigma(:,:,i));%[Time,Cluster],p(x|i)
    end
    %Compute posterior probability p(i|x)
    for j=1:nbData
        Pix(j,:) = (Priors.*Pxi(j,:))./sum(Priors.*Pxi(j,:));%p(i)*p(x|i)/(p(x)=sum_{i}(p(i)*p(x|i))),a-posterior
    end
    %Compute cumulated posterior probability
    E = sum(Pix,1);
    %% M-step %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for i=1:nbStates
        ...
        Priors(i) = E(i) / nbData;
        %Update the centers
        Mu(:,i) = Data*Pix(:,i) / E(i);%[x1,...,xN]*[p1;p2;...;pN]
        %Update the covariance matrices
