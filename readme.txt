 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_weight_transform.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* weight_transform.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* weight_transform.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* weight_transform (application created by MATLAB Compiler)
		* run_weight_transform.sh (shell script that can be run 
	...
	All Platforms:            
		* weight_transform.dll (shared library, file extension 
		  varies by platform)
		* weight_transform.h (library header file)
		* weight_transform.ctf (Component Technology File archive,
	...
	* weight_transform.xml (documentation files)
	* weight_transform.pdb (program database file - if 
	  DEBUG option selected)
	* weight_transform.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* weight_transform.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* weight_transform_1_0.dll (COM component)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
	...
	* weight_transform.jar (Java class files and compiled M-code)
	...
	* weight_transform.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* weight_transform.xla (Optionally created and placed in
