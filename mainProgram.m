...
if useHTC
    ...
    fid=fopen('speaker_signal.pcm'); xFar=fread(fid,'short'); fclose(fid);
    fid=fopen('mic_signal.pcm'); yNear=fread(fid,'short'); fclose(fid);
...
setupStruct = struct(...
    ...
    'supGain_flag', 1,...       % This flag turns on the suppression gain calculation. If turned off, gam = 1.
    'channelUpdate_flag', 1,... % This flag turns on the channel update. If turned off, H is updated for convLength and then kept constant.
    'nlp_flag', 1,...           % Turn on/off NLP
    'withVAD_flag', 1,...           % Turn on/off NLP
    ...
    'A_GAIN', 10.0,...          %
    'suppress_overdrive', 1.4,...   % overdrive factor for suppression 1.4 is good
    'gamma_echo', 1.4,...       % same as suppress_overdrive but at different place
    'de_echo_bound', 0.0,...    %
    'nl_alpha', 0.4,...         % memory; seems not very critical
    'nlSeverity', 0.5,...         % nonlinearity severity: 0 does nothing; 1 suppresses all
